...
class EGNN(nn.Module):
    ...
    def encode(self, h0, edges, edge_attr, node_mask, edge_mask, n_nodes):
        h = self.embedding(h0)
        coords = torch.randn(h.size(0), 3).to(self.device)
        for i in range(0, self.n_layers):
            if self.node_attr:
                h, coords, _ = self._modules["gcl_%d" % i](h, edges, coords, node_mask, edge_mask, edge_attr=edge_attr, node_attr=h0, n_nodes=n_nodes)
            else:
                h, _, _ = self._modules["gcl_%d" % i](h, edges, coords, node_mask, edge_mask, edge_attr=edge_attr,
                                                      node_attr=None, n_nodes=n_nodes)
            coords -= 1e-3 * coords
        adj_pred = self.decode(coords)
        return adj_pred, coords
