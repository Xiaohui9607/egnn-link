...
class ZINC(Dataset):
    def __init__(self, partition):
        self.data = torch_geometric.datasets.ZINC(root='./ae_datasets')
        # self.num_node_class = self.data.data.x.max()
        self.partition = partition
        if partition == 'train':
            self.indices = range(85000, 90000)
        if partition == 'val':
            self.indices = range(90000, 91000)
        if partition == 'test':
            self.indices = range(91000, 92000)
        self.graphs = self.create()
    def create(self):
        graphs = []
        for i in self.indices:
            ...
            G = self.data[i]
            # node_attr = [{'atom': k} for i, k in enumerate(G.x[:, :5].argmax(1).numpy().tolist())]
            G.x =torch.nn.functional.one_hot(G.x[:, 0], num_classes=28)
            # node_attr = G.x[:, :5].argmax(1).numpy().tolist()
            G = torch_geometric.utils.to_networkx(G, node_attrs=['x'], to_undirected=True)
            G = G.to_directed()
            G = gl.networkx2graph(G)
            graphs.append(G)
        return graphs
...
class Qm9(Dataset):
    def __init__(self, partition):
        self.qm9 = torch_geometric.datasets.QM9(root='./ae_datasets')
        self.partition = partition
        if partition == 'train':
            self.indices = range(85000, 90000)
        if partition == 'val':
...
if __name__ == "__main__":
    ...
    dataset = Qm9()#partition='test', overfit=True)
