...
class AE_EGNN(AE_parent):
    def __init__(self, in_n, hidden_nf, K=8, device='cpu', act_fn=nn.SiLU(), n_layers=4, reg = 1e-3, clamp=False):
        ...
        self.reg = reg
        ### Encoder
        self.add_module("gcl_0", E_GCL(in_n, self.hidden_nf, self.hidden_nf, edges_in_d=1, act_fn=act_fn, recurrent=False, clamp=clamp))
        for i in range(1, n_layers):
